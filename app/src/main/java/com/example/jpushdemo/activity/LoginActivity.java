...
public class LoginActivity extends BaseActivity {
    ...
    Observer<User> observer = new Observer<User>() {
        ...
        public void onNext(User user) {
            if (user.getCode() == 200) {
                mJpushHandler.sendMessage(mJpushHandler.obtainMessage(MSG_SET_ALIAS, "85"));
                Intent intent = new Intent(LoginActivity.this, MainActivity.class);
                intent.putExtra("username", "war3");
    ...
    private void load() {
        ...
        subscription = ApiClient.getDemoApi()
                .login(etUsername.getText().toString().trim(), etPassowrd.getText().toString().trim())
